const TaskList = () => {
  ...
  return (
    <div className="flex flex-wrap gap-6 p-6 justify-center mt-10">
      {tasks.map((task, index) => (
        <div key={index} className={`border rounded-lg shadow-md p-4 w-96 ${task.bgColor}`}>
          <div className="flex justify-between items-center">
            <button className="bg-red-500  py-1 px-6 rounded-full mb-4">{task.button}</button>
